...
const Auth = () => {
  ...
  useEffect(() => {
    const routeAfterAuth = () => {
      const newSignup = localStorage.getItem('alsa_new_signup') === '1';
      if (newSignup) {
        localStorage.removeItem('alsa_new_signup');
        navigate('/bridge-features');
      } else {
        navigate('/Chat');
      }
    };
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    localStorage.setItem('alsa_new_signup', '1');
    ...
    try {
      ...
      // If session exists (auto-confirm enabled), navigate to bridge features
      if (data.session) {
        toast({
          title: "Welcome to ALSA AI!",
          description: "Account created — chalo PC Bridge setup dekhte hain.",
        });
        navigate('/bridge-features');
      } else {
        toast({
