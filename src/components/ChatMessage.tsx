...
import { Bot, User, Copy, Loader2, ExternalLink, FileText, Music, File as FileIcon } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Prism as SyntaxHighlighter } from 'react-syntax-highlighter';
import { vscDarkPlus } from 'react-syntax-highlighter/dist/esm/styles/prism';
...
const ChatMessage = memo(({ role, content, files, keySource }: { role: 'user' | 'assistant'; content: string; files?: FileAttachment[]; keySource?: 'user' | 'server' }) => {
  const { toast } = useToast();
  const isEmpty = !content || content.trim().length === 0;
  ...
  overflow: 'hidden',        // ← ADD
      ...
      >
        {hasFiles && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: isEmpty ? 0 : 10 }}>
            {files!.map((f, i) => {
              const isImg = (f.type || '').startsWith('image/') && (f.preview || f.data);
              const isAudio = (f.type || '').startsWith('audio/');
              if (isImg) {
                return (
                  <img key={i} src={f.preview || f.data} alt={f.name}
                    style={{ width: 110, height: 110, objectFit: 'cover', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)' }} />
                );
              }
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '6px 10px', maxWidth: 220 }}>
                  {isAudio ? <Music style={{ width: 14, height: 14, color: '#a78bfa' }} /> : <FileText style={{ width: 14, height: 14, color: '#60a5fa' }} />}
                  <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                    <span style={{ fontSize: 11, color: 'white', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{f.name}</span>
                    <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)' }}>{f.type || 'file'}</span>
                  </div>
                </div>
              );
            })}
  ...
  background: 'rgba(0,0,0,0.5)'
                ...
                code({ node, inline, className, children, ...props }: any) {
                  ...
                  return !inline ? (
                    <div style={{
                      ...
                      <div style={{ width: '100%', maxWidth: '100%', overflowX: 'hidden' }}>
                        <SyntaxHighlighter
                          ...
                          codeTagProps={{ style: { fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace', whiteSpace: 'pre-wrap', wordBreak: 'break-word', overflowWrap: 'anywhere' } }}
                        >
                          {String(children).replace(/\n$/, '')}
                        </SyntaxHighlighter>
                      </div>
                    ...
                    <code
                      {...props}
                      style={{ background: 'rgba(255,255,255,0.1)', padding: '1px 6px', borderRadius: '4px', color: '#93c5fd', fontSize: '12px', wordBreak: 'break-word' }}
                    >
                      {children}
                    </code>
                  );
        ...
        {showKeyBadge && (
          <div style={{
            ...
            color: keySource === 'user' ? 'rgba(134,239,172,0.75)' : 'rgba(147,197,253,0.7)',
            display: 'flex',
            alignItems: 'center',
            gap: 6,
          }}>
            <span style={{
              ...
              background: keySource === 'user' ? '#4ade80' : '#60a5fa',
              boxShadow: `0 0 6px ${keySource === 'user' ? '#4ade80' : '#60a5fa'}`,
            }} />
