    ...
    try {
      ...
      if (providerToUse) {
        try {
          ...
          const result = await lovableAuth.signInWithOAuth(providerToUse as "google", {
            redirect_uri: `${window.location.origin}/auth?source=${providerToUse}&pre_id=${preRecord.id}`,
          ...
          if (user) {
            const payload: any = {
              user_id: user.id,
              display_name: name || user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split('@')[0] || 'ALSA AI User',
              bio,
              subscription_tier: 'free',
            };
            await safeUpsertProfiles(payload);
            markWizardCompletedFor(user.id);
          }
          setPendingOAuthProvider(null);
          setOpen(false);
          onFinish?.();
          ...
          if (avatarFile) {
            (async () => {
              try {
                ...
                if (!uploadError) {
                  const { data: signed } = await supabase.storage.from('avatars').createSignedUrl(filePath, 60 * 60 * 24 * 365 * 10);
          ...
          setSaveError(err?.message || 'Google sign-in failed. Please try again.');
          return;
        }
        ...
        try {
          ...
          if (avatarFile) {
            try {
              ...
              if (!uploadError) {
                const { data: signed } = await supabase.storage.from('avatars').createSignedUrl(filePath, 60 * 60 * 24 * 365 * 10);
                avatar_url = signed?.signedUrl || undefined;
              }
            } catch (uploadErr: any) {
      ...
      if (avatarFile) {
        try {
          ...
          payload.avatar_url = signed?.signedUrl || '';
        } catch (uploadErr: any) {
